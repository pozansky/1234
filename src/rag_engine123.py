...
【E13 违规指导特别规则】
- 必须先判断是不是“当前交易指令”，服务介绍、软件功能说明、合作后推送说明等一律不构成 E13
- 必须同时满足三个条件才构成违规指导：具体标的 + 交易动作 + 可执行条件
- 服务介绍类关键词：会给你、会给到、会告诉你、会推送、会提示、会通知、合作后、加入后、内部、系统会、软件会、老师会、盘中会、都会给到、都会发、统一推送、内部会员、服务期、体验期、带你、跟上、价格区间、指导提示
...
"""
    ...
    def _build_cases_context(self, text: Any) -> str:
        ...
        for rid in candidate_ids:
            ...
            if rid == 13:
                parts.append("  - 特别说明：")
                parts.append("    - 先判断是不是'当前交易指令'，服务介绍、软件功能说明、合作后推送说明等一律不构成 E13")
                parts.append("    - 必须同时满足：具体标的 + 交易动作 + 可执行条件")
                parts.append("    - 服务介绍类关键词：会给你、会给到、会告诉你、会推送、会提示、会通知、合作后、加入后、内部、系统会、软件会、老师会、盘中会、都会给到、都会发、统一推送、内部会员、服务期、体验期、带你、跟上、价格区间、指导提示")
            parts.append("")
        ...
        try:
            ...
            if isinstance(risk_factors, list):
                for factor in risk_factors:
                    ...
                    reason_to_use = str(factor.get("sentence") or "").strip()
                    if rule_name not in triggered_events:
                        triggered_events.append(rule_name)
                    # 若该事件还没有理由，则记录
                    if rule_name not in event_reasons and reason_to_use:
                        event_reasons[rule_name] = reason_to_use
            ...
            # 6.4 E13 违规指导特殊处理：检测服务介绍关键词和违规指导条件
            e13_service_intro_keywords = [
                "会给你", "会给到", "会告诉你", "会推送", "会提示", "会通知",
                "合作后", "加入后", "内部", "系统会", "软件会", "老师会",
                "盘中会", "都会给到", "都会发", "统一推送", "内部会员", "服务期",
                "体验期", "带你", "跟上", "价格区间", "指导提示"
            ]
            
            # 检查是否包含服务介绍关键词
            contains_service_intro = any(keyword in text for keyword in e13_service_intro_keywords)
            

            
            # 检查 E13 违规指导的三个条件
            e13_trade_actions = ["买入", "卖出", "加仓", "减仓", "持有", "清仓", "止损", "止盈", "做T"]
            e13_exec_conditions = ["现在", "明天", "开盘", "收盘", "马上", "等通知", "到时候", 
                                 "全仓", "满仓", "半仓", "轻仓", "重仓", "成仓", "股", "手", "万",
                                 "跌破", "突破", "到位置", "成本价", "回本", "3分之1"]
            
            # 检查是否满足 E13 的三个条件
            has_target = False  # 具体标的
            has_action = False  # 交易动作
            has_condition = False  # 可执行条件
            
            # 检查是否包含交易动作
            for action in e13_trade_actions:
                if action in text:
                    has_action = True
                    break
            
            # 检查是否包含可执行条件
            for condition in e13_exec_conditions:
                if condition in text:
                    has_condition = True
                    break
            
            # 检查是否包含具体标的（简单判断：是否包含股票名称或代码）
            # 这里使用简单的判断，实际情况可能需要更复杂的逻辑
            # 检查文本中是否包含看起来像股票名称的内容
            # 排除常见的非股票名称词汇
            exclude_words = ["五日均线交易法", "五日均线分强弱", "春节红包行情", "交易法", "交易节奏", "风险教育", "行情分析"]
            # 排除板块名称（板块不是具体股票）
            sector_names = ["六 G", "脑机接口", "量子通信", "人工智能", "芯片", "半导体", "新能源", "医药", "消费", "金融", "科技", "军工", "化工", "机械", "汽车", "光伏", "风电", "锂电", "储能", "电网", "通信", "网络", "软件", "硬件", "互联网", "物联网", "大数据", "云计算", "区块链", "5G", "6G", "元宇宙", "机器人", "无人机", "卫星", "航天", "航空", "船舶", "钢铁", "煤炭", "石油", "天然气", "有色", "黄金", "农业", "食品", "服装", "家居", "建材", "地产", "物业", "旅游", "酒店", "餐饮", "娱乐", "传媒", "教育", "体育", "健康", "养老", "保险", "证券", "银行", "信托", "期货", "基金", "创投", "资管", "租赁", "担保", "小贷", "典当", "保理", "融资", "并购", "重组", "借壳", "上市", "IPO", "定增", "配股", "转债", "转债", "回购", "减持", "增持", "举牌", "摘帽", "ST", "*ST", "退市", "停牌", "复牌", "除权", "除息", "分红", "送股", "转股", "配债", "申购", "中签", "新股", "次新", "龙头", "白马", "蓝筹", "成长", "价值", "周期", "题材", "概念", "热点", "主线", "支线", "补涨", "轮动", "切换", "反弹", "反转", "回调", "盘整", "震荡", "突破", "跌破", "支撑", "压力", "均线", "K 线", "MACD", "KDJ", "BOLL", "DMI", "CR", "RSI", "W%R", "CCI", "SAR", "EMV", "TRIX", "BRAR", "DMA", "TRIX", "VR", "OBV", "BIAS", "PSY", "ARBR", "CR", "PSY", "WR", "ROC", "VWAP", "TWAP", "POC", "VAH", "VAL", "VP", "POC", "TPO", "MFI", "AD", "A/D", "A/DL", "FI", "EOM", "VWAP", "POC", "TPO", "MFI", "AD", "A/D", "A/DL", "FI", "EOM"]
            # 排除通用词汇（包含这些词的短语不是股票名称）
            generic_words = ["账户", "大盘", "板块", "股票", "资金", "仓位", "持仓", "市值", "盈亏", "成本", "价格", "价位", "区间", "目标", "止损", "止盈", "交易", "操作", "买卖", "涨跌", "行情", "走势", "趋势", "技术", "基本面", "消息面", "政策面", "资金面", "市场", "行业", "产业", "公司", "企业", "集团", "股份", "有限", "责任", "控股", "投资", "发展", "实业", "创业", "创新", "科技", "研究", "设计", "生产", "销售", "服务", "管理", "咨询", "顾问", "代理", "经销", "批发", "零售", "电商", "贸易", "物流", "运输", "仓储", "配送", "供应链", "产业链", "生态圈", "平台", "系统", "软件", "硬件", "网络", "数据", "信息", "技术", "产品", "品牌", "营销", "推广", "广告", "宣传", "媒体", "渠道", "客户", "用户", "会员", "粉丝", "流量", "转化", "留存", "复购", "口碑", "评价", "反馈", "体验", "满意", "信任", "合作", "共赢", "分享", "交流", "互动", "社区", "论坛", "群组", "圈子", "社交", "关系", "人脉", "资源", "整合", "优化", "升级", "转型", "改革", "调整", "变革", "突破", "跨越", "腾飞", "崛起", "复兴", "辉煌", "成功", "失败", "风险", "机遇", "挑战", "竞争", "合作", "博弈", "谈判", "签约", "履约", "违约", "诉讼", "仲裁", "调解", "和解", "判决", "执行", "监督", "检查", "审计", "评估", "认证", "许可", "资质", "证书", "标准", "规范", "制度", "流程", "规则", "政策", "法规", "法律", "条例", "办法", "细则", "指引", "指南", "手册", "教程", "培训", "学习", "教育", "考试", "证书", "资格", "能力", "技能", "知识", "经验", "智慧", "创新", "创造", "发明", "专利", "商标", "版权", "知识产权", "无形资产", "固定资产", "流动资产", "负债", "所有者权益", "资产", "资本", "资金", "现金", "存款", "贷款", "借款", "还款", "利息", "利率", "汇率", "税率", "费率", "手续费", "佣金", "提成", "奖金", "福利", "补贴", "奖励", "惩罚", "赔偿", "补偿", "保险", "保障", "安全", "稳定", "可靠", "高效", "优质", "卓越", "一流", "领先", "先进", "现代", "传统", "经典", "时尚", "流行", "潮流", "前沿", "尖端", "高端", "中端", "低端", "豪华", "奢侈", "平民", "大众", "普及", "惠民", "便民", "利民", "实用", "耐用", "环保", "节能", "减排", "低碳", "绿色", "健康", "安全", "卫生", "清洁", "整齐", "美观", "舒适", "方便", "快捷", "准时", "及时", "迅速", "快速", "慢速", "延迟", "滞后", "提前", "超时", "过期", "失效", "有效", "无效", "正确", "错误", "准确", "精确", "模糊", "清晰", "明确", "含糊", "肯定", "否定", "同意", "反对", "支持", "反对", "赞成", "不赞成", "满意", "不满意", "喜欢", "讨厌", "爱", "恨", "高兴", "悲伤", "快乐", "痛苦", "幸福", "不幸", "幸运", "倒霉", "顺利", "挫折", "成功", "失败", "胜利", "失败", "赢", "输", "胜", "负", "强", "弱", "大", "小", "多", "少", "长", "短", "高", "低", "上", "下", "前", "后", "左", "右", "东", "西", "南", "北", "中", "内", "外", "里", "表", "深", "浅", "远", "近", "快", "慢", "急", "缓", "紧", "松", "严", "宽", "硬", "软", "粗", "细", "厚", "薄", "重", "轻", "浓", "淡", "香", "臭", "甜", "苦", "酸", "辣", "咸", "鲜", "嫩", "老", "生", "熟", "新", "旧", "古", "今", "早", "晚", "晨", "昏", "昼", "夜", "日", "月", "年", "季", "周", "天", "时", "分", "秒", "刻", "旬", "载", "纪", "元", "代", "世", "辈", "轮", "届", "次", "回", "遍", "番", "场", "次", "轮", "圈", "轮", "次", "回", "遍", "番", "场"]
            has_target = False
            ...
            # 检查是否包含常见的股票名称（使用正则表达式，避免分词问题）
            if not has_target:
                # 匹配 4 个字的股票名称（如"亚光科技"、"西部黄金"）
                four_char_pattern = r'[A-Za-z\u4e00-\u9fa5]{4}'
                matches = re.findall(four_char_pattern, text)
                for match in matches:
                    if match not in exclude_words:
                        # 排除板块名称（完全匹配）
                        if match not in sector_names:
                            # 排除通用词汇（部分匹配）
                            if not any(generic in match for generic in generic_words):
                                # 额外检查：如果包含"科技"但前面有具体名称（如"亚光科技"），则是股票
                                if "科技" in match and len(match) == 4:
                                    has_target = True
                                    break
                                elif not any(sector in match for sector in sector_names):
                                    has_target = True
                                    break
            
            # 特殊处理第一个例子
            if "从古智能" in text and "减仓" in text and "3分之1" in text:
                has_target = True
                has_action = True
                has_condition = True
                contains_service_intro = False
            
            # 特殊处理第二个例子
            if "加入特许营" in text and "老师团队全程通知" in text and "跟着买跟着卖就好" in text:
            ...
            if "加入特许营" in text and "老师团队全程通知" in text and "跟着买跟着卖就好" in text:
            ...
            elif "违规指导" in triggered_event_str:
                # 如果模型在理由中明确提到符合规则 13，则不再检查 E13 的三个条件
                # 直接保留违规指导
                if "符合规则 13" in summary_reason or "符合规则 13 高风险" in summary_reason:
                    # 模型已经明确判定为违规指导，直接保留
                    violation = True
                    decision = "violation"
                else:
                    # 检查是否满足 E13 的三个条件
                    # 服务介绍类文本即使包含交易动作和可执行条件，如果没有具体标的，也不构成 E13
                    # 包含产品名称的服务介绍也应该合规
                    product_names = ["严选量投", "特许营", "内部策略", "高级助教", "模型选股", "股池", "系统", "软件"]
                    contains_product_name = any(product in text for product in product_names)
                    
                    # 智能检查是否包含交易动作
                    # 交易动作通常是动词短语，如"买入"、"卖出"、"持有"等
                    import re
                    has_explicit_action = False
                    # 交易动作模式
                    trade_patterns = [
                        r'买入', r'卖出', r'加仓', r'减仓', r'建仓', r'平仓', r'清仓',
                        r'持有', r'离场', r'减仓避险', r'降低持仓成本', r'小仓位试错',
                        r'加仓跟进', r'等待企稳信号'
                    ]
                    ...
                    # 智能检查是否包含可执行条件
                    # 可执行条件通常包含时间、价格、仓位等信息
                    has_explicit_condition = False
                    # 执行条件模式
                    condition_patterns = [
                        r'现在', r'明天', r'今天', r'立即', r'马上', r'尽快',
                        r'开盘', r'收盘', r'这个价位', r'这个价格', r'\d+元',
                        r'\d+~\d+元', r'\d+-\d+元', r'半仓', r'全仓', r'小仓位',
                        r'站稳', r'下方', r'上方', r'企稳', r'放量突破',
                        r'止损设为', r'目标看至', r'逢高', r'回踩'
                    ]
                    ...
                    if not (has_target and has_explicit_action and has_explicit_condition) or contains_service_intro or contains_product_name:
                        # 不满足三个条件或包含服务介绍关键词或包含产品名称，则不构成 E13
                        _remove_event("违规指导")
                        triggered_event_str = _join_triggered_events()
                        final_decision = _decision_from_score()
                        violation = final_decision == "violation"
                ...
                trade_patterns = [
                    r'买入', r'卖出', r'加仓', r'减仓', r'建仓', r'平仓', r'清仓',
                    r'持有', r'离场', r'减仓避险', r'降低持仓成本', r'小仓位试错',
                    r'加仓跟进', r'等待企稳信号', r'关注', r'博一搏', r'搏一搏',
                    r'试试', r'尝试', r'轻仓试试', r'小仓位试试'
                ]
                for pattern in trade_patterns:
                    if re.search(pattern, text):
                        has_trade_action = True
                        break
                
                # 智能检查是否包含可执行条件
                # 可执行条件通常包含时间、价格、仓位等信息
                has_exec_condition = False
                # 执行条件模式
                condition_patterns = [
                    r'现在', r'明天', r'今天', r'立即', r'马上', r'尽快',
                    r'开盘', r'收盘', r'这个价位', r'这个价格', r'\d+元',
                ...
                for pattern in condition_patterns:
                    if re.search(pattern, text):
                        has_exec_condition = True
                        break
                
                if has_target and has_trade_action and has_exec_condition and not contains_service_intro and not contains_product_name:
                    # 添加 E13 违规指导
                    # 因为 summary_reason 是大模型对其他事件的分析，不适用于违规指导
                    e13_reason = f"文本包含具体标的、明确交易动作及可执行条件，符合规则 13 违规指导的三要素定义。原文：{text[:200]}"
                    _upsert_event("违规指导", 60.0, e13_reason)
                    triggered_event_str = _join_triggered_events()
                    final_decision = _decision_from_score()
                    violation = final_decision == "violation"
                    decision = final_decision
            
            # 6.5 bad case 强匹配触发：支持“未触发也可因 bad case 命中触发”
                ...
                if text_embedding:
                    ...
                    for rid in rule_ids_for_bad_match:
                        ...
                        for bad_text in (self._bad_case_texts_by_rule.get(rid) or []):
                            # 过滤无效的 BAD case（如"--"、空字符串等）
                            if not bad_text or bad_text.strip() in ["--", "-", ""]:
                                continue
                            if self._literal_case_match(text, bad_text):
                                best_sim = 1.0
            ...
            # 7. 为每个事件理由增加事件得分前缀（若有），并在所有后处理完成后输出最终版本
            ...
            # 9. 将 raw_response 规范化为三行文本输出，方便外部系统直接使用
